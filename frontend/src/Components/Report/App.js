...
const App = () => {
  ...
  useEffect(() => {
    const doFetch = async () => {
      const response = await fetch(
        `http://localhost:8000/getCacheData/bl=${user.businessLine}/region=${user.region}`
      );
  ...
  function getData(sd, ed) {
    const doFetch = async () => {
      console.log(sd, ed);
      const response = await fetch(
        `http://localhost:8000/getProcessedRecords/bl=${user.businessLine}/region=${user.region}/startDate=${sd}/endDate=${ed}`
      );
      const body = await response.json();
      const records = body;
      const user_records = records;
  ...
  return (
    <Container style={{ marginLeft: "330px" }}>
      <div style={{ paddingBottom: "0px" }}>
        <input
          style={{ width: "150px", marginTop: "40px", marginRight: "10px" }}
          name="sd"
          type="date"
      ...
      <button type="button" className="btn2" onClick={ExportToExcel}>
        Export To Excel
      </button>
